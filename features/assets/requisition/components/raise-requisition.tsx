...
> = {
    ...
    admin: [{ id: 'inventory-req', label: 'Inventory Requisition', icon: <MobileDeviceIcon /> }]
}

export function RaiseRequisition({ onBack }: { onBack: () => void }) {
    const [selectedDepartment, setSelectedDepartment] =
        useState<string>('inventory')
    ...
    const handleRemoveItem = (id: string) => {
        if (isInventoryRequisition) {
            ...
            setOfficeSuppliesItems(officeSuppliesItems.filter(item => item.id !== id))
        } else if (isEquipmentRequisition) {
        ...
        <div>
            <div>
                ...
                <Button
                    ...
                    size='sm'
                    onClick={onBack}
                    className='text-grey-700 border border-[#CCCCCC] rounded-full bg-white w-[95px] h-[39px] mb-5'
                >
                    ...
                    {!selectedRequisitionType ? (
                        <div className='flex flex-col items-center justify-center py-24 bg-[#F4F5F7] h-[70vh]'>
                            ...
                            <p
                                ...
                                className='text-neutral-black'
                            >
                                Choose a requisition type from the options above to continue
                            </p>
                        </div>
                    ) : usesFormLayout ? (
                        <div className='gap-6 mb-8'>
                            {isHRDepartment && (
                                <HRRequestForm requisitionType={selectedRequisitionType} />
                            )}
                            {isFinanceDepartment && (
                                <FinanceRequestForm requisitionType={selectedRequisitionType} />
                            )}
                            {isITDepartment && (
                                <ITRequestForm requisitionType={selectedRequisitionType} />
                            )}
                        </div>
                    ) : (
