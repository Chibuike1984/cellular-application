...
export function AddItemsPanel({ onAddItem, requisitionType }: AddItemsPanelProps) {
    const [selectedInventoryItem, setSelectedInventoryItem] = useState<string>('') // separate state
    const [selectedOfficeItem, setSelectedOfficeItem] = useState<string>('')       // separate state
    ...
    const handleAdd = () => {
        const itemName = isInventoryRequisition
            ? selectedInventoryItem
            : isOfficeSupplies
            ? selectedOfficeItem
            : itemDescription

        if (!itemName || (showQuantityField && !quantity)) return

        const itemToAdd = isInventoryRequisition
            ? {
                  itemName,
                  currentStock: inventoryItems.find(i => i.name === selectedInventoryItem)?.stock || '-',
                  reorderLevel: inventoryItems.find(i => i.name === selectedInventoryItem)?.reorder || '-',
    ...
    return (
        <div className='bg-white border border-grey-200'>
            <div className='flex items-center justify-between mb-6 bg-[#F2F2F2] h-[62px] px-5'>
                <h2 className='text-[16px] font-semibold'>Add Items</h2>
            </div>

            <div className='space-y-4 px-5'>
                {isInventoryRequisition && (
                    <div>
                        <label className='text-[#3E3E3E] block mb-2 text-[13px] font-normal'>
                            Select Item
                        </label>
                ...
                {isOfficeSupplies && (
                    <div>
                        ...
                        <Select value={selectedOfficeItem} onValueChange={setSelectedOfficeItem}>
                            <SelectTrigger className='w-full border-grey-200 rounded-full'>
                                <SelectValue placeholder='Select an item...' />
                            </SelectTrigger>
                            <SelectContent>
                ...
                {!isInventoryRequisition && !isOfficeSupplies && (
                    <div>
                        <label className='text-[#3E3E3E] block mb-2 text-[13px] font-normal'>
                            Item Description
                        </label>
